...
- Dilarang: torrent/P2P, remote proxy, crypto mining, DoS attack, password cracking, deepfake, containerization untuk membypass kebijakan, **DAN "using multiple accounts to work around access or resource usage restrictions"** (dikonfirmasi ulang 2026-08-22, kutipan langsung FAQ resmi) — **⚠️ RELEVAN LANGSUNG utk sistem ini**: user py banyak akun Google berbeda (warungbudina, gogobuda65, maydualapan8, balibruntattour, clawapp193, dst — lihat berbagai memori proyek) yg semuanya numpang infra yg SAMA (laptop/WireGuard/IP yg sama). **JANGAN sebar beban Colab ke akun-akun ini bergantian utk melipatgandakan kuota** — pola itu PERSIS yg dilarang eksplisit, dan deteksi Google memakai *cross-account fraud linkage* yg justru DIPERMUDAH krn akun-akun ini sudah terbukti berbagi jejak jaringan yg sama (beda drpd pola Cloud Shell yg memang didesain 1-akun-1-VM independen tanpa larangan serupa).
- Tier gratis khusus tambahan larangan (berlaku selama TANPA saldo compute unit positif, bisa diakhiri kapan saja tanpa peringatan): **SSH shell / remote desktop**, membypass UI notebook lewat web interface lain, worker distributed computing, chess training(!). **⚠️ RELEVAN LANGSUNG:** seluruh gaya operasi sistem ini adalah "orchestrator di akses-vps kendalikan node lewat SSH/API sbg remote worker" (pola sama persis dgn Cloud Shell/Pi/RN7) — **pola itu TIDAK BOLEH diterapkan ke Colab tier gratis**. Kalau mau Colab dipanggil programatik dari luar (mis. trigger training dari akses-vps tanpa manusia buka notebook), itu masuk kategori "bypassing the notebook UI to interact primarily via a web UI" yg eksplisit dilarang di tier gratis.
- **Kebijakan "cool down" (ditemukan 2026-08-22):** Google secara eksplisit menyebut evaluasi berbasis pola pemakaian historis — **akun dgn riwayat pemakaian rutin/wajar dapat kuota run lebih panjang drpd akun yg baru saja dipakai "abusively"** (definisi "abusively" tak dipublikasikan detail). Praktis: pemakaian **konsisten & moderat** (kerja pendek rutin) lebih menguntungkan jangka panjang drpd pemakaian sporadis-tapi-ekstrem (nganggur berminggu lalu tiba-tiba pakai GPU nonstop berjam-jam).
- **Bukti nyata risiko suspensi (bukan cuma teori):** ada laporan publik developer (GitHub issue `googlecolab/colabtools`) akun ter-blokir *"suspected abusive activity"* — salah satu kasus terdokumentasi krn pakai **ngrok tunnel**, akun tersuspend **berbulan-bulan**. Larangan-larangan ini melonggar sedikit di paket berbayar selama saldo compute positif, tapi larangan "web service offering 24/7" pada dasarnya tetap berlaku secara semangat ToS — resource Colab diprioritaskan untuk *interactive use case*, bukan backend produksi.
...
- **Kaggle Notebooks sebagai alternatif**: Kaggle memberi kuota GPU mingguan (dilaporkan **~30 jam GPU/minggu**, sesi maks ~12 jam, GPU T4/P100 + akses TPU) secara gratis tanpa kartu kredit, dan sekarang **Colab Pro bisa ditautkan ke akun Kaggle untuk menambah kuota GPU mingguan di Kaggle**. Cocok dipakai bergantian dengan Colab kalau kuota salah satu habis. **Keunggulan Kaggle utk perencanaan jangka panjang: kuotanya DIPUBLIKASIKAN & tetap** (~30 jam/minggu pasti), beda dari Colab yg sengaja TAK mempublikasikan angka kuota GPU-nya (dinamis, tergantung demand+riwayat akun) — kalau butuh kepastian kuota utk jadwal kerja rutin, Kaggle lebih bisa direncanakan drpd Colab.
- **⚠️ Background execution Pro/Pro+ TAK SEPENUHNYA ANDAL** (ditemukan 2026-08-22): meski fitur ini resminya "runtime lanjut jalan sampai 24 jam meski browser ditutup" utk Pro+, banyak laporan resmi (GitHub issue `googlecolab/colabtools` #4151/#5950/#4666/#4874) runtime tetap terhenti dini (40mnt-1 jam) atau opsi "Background Execution" tak muncul sama sekali meski sudah bayar. **Jangan asumsikan bayar Pro+ = job pasti selesai tanpa diawasi** — tetap desain job supaya checkpoint sering + toleran gagal-tengah-jalan, sama seperti di tier gratis.
